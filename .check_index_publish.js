    ...
    async function installPWA(){
      if (!deferredPrompt){
        alert("Install is not ready yet. Open this app in a supported browser and wait a few seconds.");
        return;
      }
    ...
    async function handleStudentLogin(){
      ...
      if (!matchedStudent) {
        ...
        if (primaryMatches.length) {
            ...
            if (authSync && typeof authSync.verifyStudentLogin === "function") {
              try {
                ...
                if (remoteStudent) {
                  ...
                  setLocalStudents(getLocalStudents().concat(matchedStudent));
                }
              } catch (_) {}
            }
          }
          ...
          if (!matchedStudent) {
            studentLoginStatus.innerHTML = `<span class="danger">Student ID or PIN not found.${buildCloudHint(await getAuthSync())}</span>`;
            safePlay("warning");
    ...
    async function handleTeacherSignup(){
      ...
      const authSync = await getAuthSync();
      const existingTeacher = findTeacherByEmail(email);
      let remoteTeacher = null;
      if (authSync && typeof authSync.getTeacherAccount === "function") {
        try {
          remoteTeacher = await authSync.getTeacherAccount(email);
        } catch (_) {}
      }

      const teacherRecord = normalizeTeacherRecord({
        ...(remoteTeacher || {}),
        ...(existingTeacher || {}),
        id: existingTeacher?.id || remoteTeacher?.id || ("T-" + Date.now()),
        name,
        school,
        email,
        password,
        createdAt: existingTeacher?.createdAt || remoteTeacher?.createdAt || new Date().toISOString()
      });
      ...
      if (authSync && typeof authSync.saveTeacherAccount === "function") {
        try {
          await authSync.saveTeacherAccount(teacherRecord);
        } catch (_) {}
      }
      ...
      const restored = !!(existingTeacher || remoteTeacher);
      teacherSignupStatus.innerHTML = restored
      ...
      teacherLoginPassword.value = password;
      teacherSignupPassword.value = "";
      safePlay("success");
      FX.toast(restored ? "Teacher account restored." : "Teacher signup successful.", "👩‍🏫");
    ...
    async function handleTeacherLogin(){
      ...
      if (!email || !password) {
        ...
        return;
          ...
          reconcileAuthRecords();
          const authSync = await getAuthSync();
          let teacherCandidates = getTeacherCandidatesByEmail(email);
          let teacher = teacherCandidates.find(t => String(t.password || "") === password) || teacherCandidates[0] || null;
          let remoteTeacher = null;

          if (!teacher) {
            const relatedStudents = getLocalStudents().filter(s => normalizeEmail(s && s.teacherEmail) === email);
            if (relatedStudents.length) {
            const fallbackTeacher = normalizeTeacherRecord({
              id: "T-RECOVER-" + email,
              email,
              teacherName: relatedStudents[0].teacherName || "",
              school: relatedStudents[0].classId || "",
              password,
              createdAt: relatedStudents[0].createdAt || new Date().toISOString()
            });
              setTeachers(getTeachers().concat(fallbackTeacher));
              teacherCandidates = getTeacherCandidatesByEmail(email);
              teacher = teacherCandidates.find(t => String(t.password || "") === password) || teacherCandidates[0] || null;
            }
          }

          if ((!teacher || String(teacher.password || "") !== password)) {
            const rememberedTeacher = getRememberedCredential("teacher", email);
            if (rememberedTeacher && String(rememberedTeacher.password || "") === password) {
              teacher = normalizeTeacherRecord({
                ...(teacher || {}),
                id: teacher?.id || ("T-CACHE-" + email),
                name: teacher?.name || email.split("@")[0] || "Teacher",
      ...
      if (teacher && !String(teacher.password || "").trim()) {
        const teachers = getTeachers().filter(t => normalizeEmail(t.email) !== email);
        teacher = {
          ...teacher,
          password
        };
        ...
        setTeachers(teachers);
          ...
          if ((!teacher || String(teacher.password || "") !== password) && authSync) {
            if (typeof authSync.verifyTeacherLogin === "function") {
              try {
                remoteTeacher = await authSync.verifyTeacherLogin(email, password);
              } catch (_) {}
            }
            if (!remoteTeacher && typeof authSync.getTeacherAccount === "function") {
              try {
                remoteTeacher = await authSync.getTeacherAccount(email);
              } catch (_) {}
            }
            if (remoteTeacher) {
              const passwordVerified =
                !remoteTeacher.passwordHash ||
                (typeof authSync.verifyTeacherLogin === "function"
                  ? !!(await authSync.verifyTeacherLogin(email, password).catch(() => null))
                  : true);
              if (passwordVerified) {
                teacher = normalizeTeacherRecord({
                  ...(teacher || {}),
                  ...remoteTeacher,
                  id: teacher?.id || remoteTeacher.id || ("T-" + Date.now()),
                  name: teacher?.name || remoteTeacher.name || email.split("@")[0],
                  school: teacher?.school || remoteTeacher.school || "",
                  email,
                  password
                });
                setTeachers(getTeachers().filter(t => normalizeEmail(t.email) !== email).concat(teacher));
          ...
          if (!teacher || String(teacher.password || "") !== password) {
            const hasRemoteTeacher = !!remoteTeacher;
            teacherLoginStatus.innerHTML = hasRemoteTeacher
              ? `<span class="danger">Teacher password is incorrect for that email.</span>`
              : `<span class="danger">Teacher account not found or password is incorrect.${buildStorageHint("teacher", email)}${buildCloudHint(authSync)}</span>`;
            safePlay("warning");
        ...
        setTeachers(getTeachers().filter(t => normalizeEmail(t.email) !== email).concat({
          ...teacher,
          email,
          password
        }));
      ...
      clearParentSession();
        ...
        const teacherSync = await getAuthSync();
        if (teacherSync && typeof teacherSync.saveTeacherAccount === "function") {
          try {
            await teacherSync.saveTeacherAccount({
              ...teacher,
              email,
              password
            });
    ...
    async function handleTeacherForgotPassword(){
      const email = normalizeEmail(teacherLoginEmail.value);
      if (!email) {
        teacherLoginStatus.innerHTML = `<span class="danger">Enter your teacher email first to retrieve the saved password.</span>`;
        safePlay("warning");
        return;
      }

        reconcileAuthRecords();
        const teacher = getTeacherCandidatesByEmail(email).find(t => String(t.password || "").trim()) || findTeacherByEmail(email);
        if (!teacher) {
          const authSync = await getAuthSync();
          if (authSync && typeof authSync.getTeacherAccount === "function") {
            try {
              const remoteTeacher = await authSync.getTeacherAccount(email);
              if (remoteTeacher) {
                teacherLoginStatus.innerHTML = `<span class="danger">Teacher account exists in cloud sync, but no readable password is saved on this device yet. Login once on this device with your password, then Forgot Password will work here afterwards.</span>`;
                safePlay("warning");
                return;
              }
            } catch (_) {}
          }
          teacherLoginStatus.innerHTML = `<span class="danger">No teacher account was found for that email.${buildStorageHint("teacher", email)}${buildCloudHint(authSync)}</span>`;
          safePlay("warning");
          FX.toast("Teacher email not found.", "❌");
          return;
        }

      if (!String(teacher.password || "").trim()) {
        teacherLoginStatus.innerHTML = `<span class="danger">Teacher email found, but no saved password exists yet. Enter a new password in the login box and click Login Teacher to restore the account.</span>`;
        safePlay("warning");
        FX.toast("Teacher account found. Set a new password to restore it.", "🔐");
        return;
      }

      teacherLoginPassword.value = String(teacher.password || "");
      teacherLoginStatus.innerHTML = `<span class="ok">Saved password restored for ${teacher.email}: <b>${teacher.password || ""}</b></span>`;
      safePlay("success");
      ...
      async function handleTeacherResetPassword(){
        const email = normalizeEmail(teacherLoginEmail.value);
        const newPassword = (teacherLoginPassword.value || "").trim();
        if (!email) {
          teacherLoginStatus.innerHTML = `<span class="danger">Enter your teacher signup email first.</span>`;
          safePlay("warning");
          return;
        }
        if (newPassword.length < 6) {
          teacherLoginStatus.innerHTML = `<span class="danger">Enter a new teacher password with at least 6 characters in the password box.</span>`;
          safePlay("warning");
          return;
        }

        reconcileAuthRecords();
        let teacher = findTeacherByEmail(email);
        const authSync = await getAuthSync();
        let remoteTeacher = null;
        if (authSync && typeof authSync.getTeacherAccount === "function") {
          try {
        ...
        if (!teacher && !remoteTeacher) {
          teacherLoginStatus.innerHTML = `<span class="danger">No teacher account was found for that email.</span>`;
          safePlay("warning");
          FX.toast("Teacher email not found.", "❌");
          return;
        }
        ...
        teacher = normalizeTeacherRecord({
          ...
          createdAt: teacher?.createdAt || remoteTeacher?.createdAt || new Date().toISOString()
        });
        ...
        if (authSync && typeof authSync.saveTeacherAccount === "function") {
          try {
            await authSync.saveTeacherAccount(teacher);
          } catch (_) {}
        }

        teacherLoginStatus.innerHTML = `<span class="ok">Teacher password reset successful. You can now login on this or another device.</span>`;
        safePlay("success");
        FX.toast("Teacher password reset.", "🔐");
      }
    ...
    async function handleParentSignup(){
      ...
      if (authSync && typeof authSync.getParentAccount === "function") {
        try {
          remoteParent = await authSync.getParentAccount(email);
        } catch (_) {}
      }

      const parentRecord = normalizeParentRecord({
        ...(remoteParent || {}),
        ...(existingParent || {}),
        id: existingParent?.id || remoteParent?.id || ("P-" + Date.now()),
        name,
        childName,
        childStudentId,
        email,
        password,
        createdAt: existingParent?.createdAt || remoteParent?.createdAt || new Date().toISOString()
      });
      ...
      if (authSync && typeof authSync.saveParentAccount === "function") {
        try {
          await authSync.saveParentAccount(parentRecord);
        } catch (_) {}
      }

      const resolvedLevel = String(
        inferLevelFromStudentId(parentRecord.childStudentId) ||
        "A"
      ).toUpperCase();
      setParentSession({
        id: parentRecord.id,
      ...
      const restored = !!(existingParent || remoteParent);
      parentSignupStatus.innerHTML = restored
      ...
      parentLoginPassword.value = password;
      parentSignupPassword.value = "";
      safePlay("success");
      FX.toast(restored ? "Parent account restored." : "Parent signup successful.", "👨‍👩‍👧");
      speakAnnouncement("Welcome to GP EIS Online Learning. A bright and professional learning home for students.");
    }
      ...
      if (!email || !password) {
        ...
        return;
          ...
          reconcileAuthRecords();
          const authSync = await getAuthSync();
          let parentCandidates = getParentCandidatesByEmail(email);
        let parent = parentCandidates.find(p => String(p.password || "") === password) || parentCandidates[0] || null;
        let remoteParent = null;

        if ((!parent || String(parent.password || "") !== password)) {
          const rememberedParent = getRememberedCredential("parent", email);
          if (rememberedParent && String(rememberedParent.password || "") === password) {
            parent = normalizeParentRecord({
              ...(parent || {}),
              id: parent?.id || ("P-CACHE-" + email),
              name: parent?.name || email.split("@")[0] || "Parent",
      ...
      if (parent && !String(parent.password || "").trim()) {
        const parents = getParents().filter(p => normalizeEmail(p.email) !== email);
        parent = {
          ...parent,
          password
        };
        parents.push(parent);
          setParents(parents);
          parentCandidates = getParentCandidatesByEmail(email);
          parent = parentCandidates.find(p => String(p.password || "") === password) || parentCandidates[0] || null;
        }

        if ((!parent || String(parent.password || "") !== password) && authSync) {
            if (typeof authSync.verifyParentLogin === "function") {
              try {
                remoteParent = await authSync.verifyParentLogin(email, password);
              } catch (_) {}
            }
            if (!remoteParent && typeof authSync.getParentAccount === "function") {
              try {
                remoteParent = await authSync.getParentAccount(email);
              } catch (_) {}
            }
            if (remoteParent) {
              const passwordVerified =
                !remoteParent.passwordHash ||
                (typeof authSync.verifyParentLogin === "function"
                  ? !!(await authSync.verifyParentLogin(email, password).catch(() => null))
                  : true);
              if (passwordVerified) {
                parent = normalizeParentRecord({
                  ...(parent || {}),
        ...
        if (!parent || String(parent.password || "") !== password) {
          const hasRemoteParent = !!remoteParent;
          parentLoginStatus.innerHTML = hasRemoteParent
            ? `<span class="danger">Parent password is incorrect for that email.</span>`
            : `<span class="danger">Parent account not found or password is incorrect.${buildStorageHint("parent", email)}${buildCloudHint(authSync)}</span>`;
          safePlay("warning");
      ...
      storageSetItem(STORAGE_KEYS.level, resolvedLevel);
        setParents(getParents().filter(p => normalizeEmail(p.email) !== email).concat({
          ...parent,
          email,
          password
        }));
      ...
      clearTeacherSession();
        ...
        speakAnnouncement("Welcome to GP EIS Online Learning. A bright and professional learning home for students.");
        const parentSync = await getAuthSync();
        if (parentSync && typeof parentSync.saveParentAccount === "function") {
          try {
            await parentSync.saveParentAccount({
              ...parent,
              email,
              password
            });
    ...
    async function handleParentForgotPassword(){
      const email = normalizeEmail(parentLoginEmail.value);
      if (!email) {
        parentLoginStatus.innerHTML = `<span class="danger">Enter your parent email first to retrieve the saved password.</span>`;
        safePlay("warning");
        return;
      }

        reconcileAuthRecords();
        const parent = getParentCandidatesByEmail(email).find(p => String(p.password || "").trim()) || findParentByEmail(email);
        if (!parent) {
          const authSync = await getAuthSync();
          if (authSync && typeof authSync.getParentAccount === "function") {
            try {
              const remoteParent = await authSync.getParentAccount(email);
              if (remoteParent) {
                parentLoginStatus.innerHTML = `<span class="danger">Parent account exists in cloud sync, but no readable password is saved on this device yet. Login once on this device with your password, then Forgot Password will work here afterwards.</span>`;
                safePlay("warning");
                return;
              }
            } catch (_) {}
          }
          parentLoginStatus.innerHTML = `<span class="danger">No parent account was found for that email.${buildStorageHint("parent", email)}${buildCloudHint(authSync)}</span>`;
          safePlay("warning");
          FX.toast("Parent email not found.", "❌");
          return;
        }

      if (!String(parent.password || "").trim()) {
        parentLoginStatus.innerHTML = `<span class="danger">Parent email found, but no saved password exists yet. Enter a new password in the login box and click Login Parent to restore the account.</span>`;
        safePlay("warning");
        FX.toast("Parent account found. Set a new password to restore it.", "🔐");
        return;
      }

      parentLoginPassword.value = String(parent.password || "");
      parentLoginStatus.innerHTML = `<span class="ok">Saved password restored for ${parent.email}: <b>${parent.password || ""}</b></span>`;
      safePlay("success");
      ...
      async function handleParentResetPassword(){
        const email = normalizeEmail(parentLoginEmail.value);
        const newPassword = (parentLoginPassword.value || "").trim();
        if (!email) {
          parentLoginStatus.innerHTML = `<span class="danger">Enter your parent signup email first.</span>`;
          safePlay("warning");
          return;
        }
        if (newPassword.length < 6) {
          parentLoginStatus.innerHTML = `<span class="danger">Enter a new parent password with at least 6 characters in the password box.</span>`;
          safePlay("warning");
          return;
        }

        reconcileAuthRecords();
        let parent = findParentByEmail(email);
        const authSync = await getAuthSync();
        let remoteParent = null;
        if (authSync && typeof authSync.getParentAccount === "function") {
          try {
        ...
        if (!parent && !remoteParent) {
          parentLoginStatus.innerHTML = `<span class="danger">No parent account was found for that email.</span>`;
          safePlay("warning");
          FX.toast("Parent email not found.", "❌");
          return;
        }
        ...
        parent = normalizeParentRecord({
          ...(parent || {}),
          ...(remoteParent || {}),
          id: parent?.id || remoteParent?.id || ("P-" + Date.now()),
          name: parent?.name || remoteParent?.name || email.split("@")[0],
          childName: parent?.childName || remoteParent?.childName || "",
          childStudentId: parent?.childStudentId || remoteParent?.childStudentId || "",
          email,
          password: newPassword,
          createdAt: parent?.createdAt || remoteParent?.createdAt || new Date().toISOString()
        });
        ...
        if (authSync && typeof authSync.saveParentAccount === "function") {
          try {
            await authSync.saveParentAccount(parent);
          } catch (_) {}
        }

        parentLoginStatus.innerHTML = `<span class="ok">Parent password reset successful. You can now login on this or another device.</span>`;
        safePlay("success");
        FX.toast("Parent password reset.", "🔐");
      }

    ...
    async function handleStudentLogin(){
      ...
      if (!matchedStudent) {
        ...
        if (primaryMatches.length) {
          ...
          if (!matchedStudent) {
            studentLoginStatus.innerHTML = `<span class="danger">Student ID or PIN not found.</span>`;
            safePlay("warning");
    ...
    async function handleTeacherSignup(){
      ...
      if (existingTeacher && String(existingTeacher.password || "").trim()) {
        teacherSignupStatus.innerHTML = `<span class="danger">This teacher email already exists.</span>`;
        safePlay("warning");
        return;
      }

      const nextTeachers = teachers.filter(t => normalizeEmail(t.email) !== email);
      nextTeachers.push({
        id: existingTeacher?.id || ("T-" + Date.now()),
        name,
        school,
        email,
        password,
        createdAt: existingTeacher?.createdAt || new Date().toISOString()
      });
        ...
        if (authSync && typeof authSync.saveTeacherAccount === "function") {
          try {
            await authSync.saveTeacherAccount({
              id: existingTeacher?.id || ("T-" + Date.now()),
              name,
              school,
              email,
        ...
        teacherSignupStatus.innerHTML = `<span class="ok">Teacher signup successful. You can login now from this or another device.</span>`;
      teacherLoginEmail.value = email;
      teacherSignupPassword.value = "";
      safePlay("success");
      FX.toast("Teacher signup successful.", "👩‍🏫");
    }
        ...
        setTeachers(teachers);
          ...
          if (!teacher || String(teacher.password || "") !== password) {
            ...
            teacherLoginStatus.innerHTML = hasRemoteTeacher
              ...
              : `<span class="danger">Teacher account not found or password is incorrect.${buildStorageHint("teacher", email)}</span>`;
            safePlay("warning");
    ...
    async function handleTeacherForgotPassword(){
      ...
      if (!email) {
        ...
        if (!teacher) {
          ...
          teacherLoginStatus.innerHTML = `<span class="danger">No teacher account was found for that email.${buildStorageHint("teacher", email)}</span>`;
          safePlay("warning");
    ...
    async function handleParentSignup(){
      ...
      if (existingParent && String(existingParent.password || "").trim()) {
        parentSignupStatus.innerHTML = `<span class="danger">This parent email already exists.</span>`;
        safePlay("warning");
        return;
      }

        const nextParents = parents.filter(p => normalizeEmail(p.email) !== email);
      nextParents.push({
        ...
        if (authSync && typeof authSync.saveParentAccount === "function") {
          try {
            await authSync.saveParentAccount({
              id: existingParent?.id || ("P-" + Date.now()),
              name,
              childName,
              childStudentId,
        ...
        parentSignupStatus.innerHTML = `<span class="ok">Parent signup successful. You can login now from this or another device.</span>`;
      parentLoginEmail.value = email;
      parentSignupPassword.value = "";
      safePlay("success");
      FX.toast("Parent signup successful.", "👨‍👩‍👧");
    }
      ...
      if (parent && !String(parent.password || "").trim()) {
        ...
        if (!parent || String(parent.password || "") !== password) {
          ...
          parentLoginStatus.innerHTML = hasRemoteParent
            ...
            : `<span class="danger">Parent account not found or password is incorrect.${buildStorageHint("parent", email)}</span>`;
          safePlay("warning");
    ...
    async function handleParentForgotPassword(){
      ...
      if (!email) {
        ...
        if (!parent) {
          ...
          parentLoginStatus.innerHTML = `<span class="danger">No parent account was found for that email.${buildStorageHint("parent", email)}</span>`;
          safePlay("warning");
    ...
    function logoutUser(){
      ...
      clearTeacherSession();
      clearParentSession();
      localStorage.removeItem(STORAGE_KEYS.authGate);
      selectedRole = "student";
      storageSetItem(STORAGE_KEYS.role, "student");
      syncLevelUI();
